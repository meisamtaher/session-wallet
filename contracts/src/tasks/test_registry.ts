...
import { getPlugin, getRegistry, getRelayPlugin, getSessionPlugin } from "../utils/contracts";
...
task("register-plugin", "Registers the sample Plugin in the Safe{Core} test register")
    .setAction(async (_, hre) => {
        ...
        const plugin = await getSessionPlugin(hre)
        await registry.addIntegration(await plugin.getAddress(), IntegrationType.Plugin)
        console.log("Registered Plugin registry Session Plugin")
    });
